...
// The below is handled in the runtb_*.tcl TCL script.
...
// Chip slow-control DUT
slow_control_256 U_DUT(
...
begin
	test_pass = 1;
	// Logistics
	tbfile = $fopen("slow_control_256_tb.results", "a");	
	...
	`ifdef SIM_POSTSYNTH
		...
		$sdf_annotate("./../../syn/output/slow_control_256.syn.sdf", "U_DUT", , , "MAXIMUM", , );	// There is only one corner in synthesis which is the worst delay
	`endif
	`ifdef SIM_POSTPNR
		$display("Post-PNR simulation");
		$fwrite(tbfile, "Post-PNR simulation\n");
		$sdf_annotate("./../../pnr/output/slow_control_256.funct.sdf", "U_DUT", , , `SDF_CORNER, , );
