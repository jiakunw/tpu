# project_setup_bias.tcl
# bias_wrapper: wraps sram01 (512 x 32-bit)
# Bus: write only (CPU writes INT32 bias, 4 bytes per row)
# TPU: read only (32-bit per cycle)
...
#####################################################################
set TOPCELL             bias_wrapper
set vars(std_height)    1.8

set gn::VERILOG_LIST    [list "bias_wrapper.sv"]
set gn::SDC_LIST        [glob ${gn::SDCDir}*.sdc]
set iv::node            65

#####################################################################
# Project Information, Floorplanning
# sram01 R0 dims are smaller than sram00 (32-bit vs 64-bit wide)
# Using same fp width as sram_wrapper_c; height slightly smaller.
# Adjust vars(fp,height) once sram01 actual dims are confirmed.
#####################################################################
...
#### die dimensions ####
# sram01 is narrower than sram00; use same fp width, slightly shorter height
# TODO: update once sram01 actual LEF dimensions are confirmed
set vars(fp,width)              155.0
set vars(fp,height)             [expr {180 * $vars(std_height)}]
...
# PDK Information
#####################################################################
set env(SRAM_DIR) "/courses/ee6350/proj_2026Spring/team09/Chenting_Systolic/TinyTPU_v5/memory_compiler/sram01_gen"
...
set gn::NLDMLIB [list \
    ...
    "$SRAM_DIR/sram01_nldm_ss_0p90v_0p90v_125c_syn.lib" ]

#####################################################################
# PDK Information, Place & Route
#####################################################################
set iv::OALIB           [list "tcbn65gplus_oalib" "sram01_oalib"]
set iv::OALIB_COSTUM    [list "sram01_oalib"]
...
#####################################################################
# SRAM Hard Macro (sram01)
# TODO: update actual dims once sram01 LEF is available
#####################################################################
set iv::SRAM_LEF        "$SRAM_DIR/sram01.vclef"
set iv::SRAM_GDS        "$SRAM_DIR/sram01.gds2"
set iv::SRAM_CDL        "$SRAM_DIR/sram01.cdl"

set iv::SRAM_LIB_TYP    "$SRAM_DIR/sram01_nldm_tt_1p00v_1p00v_25c_syn.lib"
set iv::SRAM_LIB_MIN    "$SRAM_DIR/sram01_nldm_ff_1p10v_1p10v_0c_syn.lib"
set iv::SRAM_LIB_MIN_LT "$SRAM_DIR/sram01_nldm_ff_1p10v_1p10v_m40c_syn.lib"
set iv::SRAM_LIB_MAX    "$SRAM_DIR/sram01_nldm_ss_0p90v_0p90v_125c_syn.lib"

set iv::BLACKBOX_LIST {sram01}
set iv::MACRO_V_LIST [list "$SRAM_DIR/sram01.v"]

###############################################################################
# HARD MACRO PLACEMENT
# TODO: replace with actual sram01 dimensions from LEF
# Using sram00 dims as placeholder: W=312.26um H=128.765um R0
###############################################################################
set vars(sram,inst_name)    "u_sram"
set vars(sram,x)            $vars(fp,io_core_space)
set vars(sram,y)            $vars(fp,io_core_space)
set vars(sram,orient)       "R90"
